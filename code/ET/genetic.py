...
for filename in os.listdir(folder_path):
    if filename.endswith('.csv'):
        ...
        # 유전 알고리즘 설정
        varbound = np.array([[50, 300], [3, 15], [2, 10], [1, 4], [0, 1]])
        ...
        model = ga(function=fitness_function, dimension=5, variable_type='int', variable_boundaries=varbound, algorithm_parameters=algorithm_param)
        model.run()
        ...
        params_df = pd.DataFrame({
            ...
            'Parameter': ['n_estimators', 'max_depth', 'min_samples_split', 'min_samples_leaf', 'bootstrap'],
            'Best Value': [n_estimators, max_depth, min_samples_split, min_samples_leaf, bootstrap]
        })
