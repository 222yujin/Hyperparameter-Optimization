...
for filename in os.listdir(folder_path):
    if filename.endswith('.csv'):
        ...
        # 파일을 잠시 기다렸다가 열기 (다른 프로세스에서 열려있는 문제 방지)
        time.sleep(5)
        ...
        if not os.path.exists(output_file):
            ...
            with pd.ExcelWriter(output_file, engine='openpyxl', mode='a', if_sheet_exists='overlay') as writer:
                ...
                if performance_metrics_sheet in writer.book.sheetnames:
                    startrow = writer.book[performance_metrics_sheet].max_row
                    results_df.to_excel(writer, sheet_name=performance_metrics_sheet, index=False, startrow=startrow, header=False)
                else:
                ...
                if best_hyperparams_sheet in writer.book.sheetnames:
                    startrow = writer.book[best_hyperparams_sheet].max_row
                    params_df.to_excel(writer, sheet_name=best_hyperparams_sheet, index=False, startrow=startrow, header=False)
                else:
                ...
                if classification_report_sheet in writer.book.sheetnames:
                    startrow = writer.book[classification_report_sheet].max_row
                    report_df.to_excel(writer, sheet_name=classification_report_sheet, index=True, startrow=startrow, header=False)
                else:
